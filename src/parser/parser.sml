...
structure Parser : sig
    ...
    (* parse an Ovid program from an input stream using the given error stream to record errors *)
    ...
    structure OvidParser = OvidParseFn(OvidLex)
    ...
    fun parse (inStrm, errStrm) = let
          ...
          val lexer = OvidLex.lex (Error.sourceMap errStrm) (lexErr errStrm)
          val (yield, _, errs) = OvidParser.parse lexer (OvidLex.streamify get)
          in
