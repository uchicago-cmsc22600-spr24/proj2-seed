  ...
  (* conditionally wrap a SPAN around a subtree *)
    ...
    and dcl2sexp (marks, dcl) = (case dcl
	   of PT.MarkDcl m => mark2sexp dcl2sexp (marks, m)
	    | PT.OverloadDcl(id, ty) =>
                mkOverloadDcl [SYMBOL id, ovldty2sexp(marks, ty)]
	    | PT.InstanceDcl(id, ty, id') => mkInstanceDcl [
		  SYMBOL id, ovldty2sexp(marks, ty), SYMBOL id'
		]
    ...
    and ovldty2sexp (marks, ty) = let
	  fun toSExp ty = (case ty
		 of PT.MarkOvldTy m => mark2sexp ovldty2sexp (marks, m)
                  | PT.OvldTy(tyParams, ty) => mkOvldTy [
                        LIST(List.map SYMBOL tyParams), ty2sexp (marks, ty)
                      ]
                (* end case *))
          in
            toSExp ty
